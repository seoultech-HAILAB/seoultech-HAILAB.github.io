...
크기는 **아홉 역할뿐**이다. CSS에 px를 직접 적지 않는다 — `font-size: var(--t-…)` 만 쓴다.
새 자리가 생겨도 열 번째 크기를 만들지 않는다. 아래 표에서 가장 가까운 역할을 고른다.

| 이름 | 값 | 역할 | 어디에 |
|---|---|---|---|
| `--t-hero` | 46px | 첫 화면 제목 | 히어로 제목 하나뿐 |
| `--t-page` | 32px | 페이지 제목 | `Publications`, `Projects`, 글 상세의 제목 |
| `--t-sec` | 24px | 구역 제목 | `› Faculty`, `What We Explore`, 하이라이트 제목 |
| `--t-item` | 18px | 항목 제목 (굵게) | 카드 제목, 논문 제목, 목록 한 줄의 제목, 이름 |
| `--t-body` | 17px | 본문 | 글 본문 문단, 소개 문단, 상단 메뉴 |
| `--t-meta` | 15px | 보조 줄 | 날짜, 저자, 학회명, 역할, 소속, 카드의 한 줄 설명, 기간, 금액 |
| `--t-small` | 15px | 작은 글 | 버튼, 필터, 캡션, 주석, 개수, 푸터 |
| `--t-label` | 12px | 라벨 | 태그, 뱃지, 대문자 소제목(`OUR RESEARCH`), 칩 |
| `--t-num` | 34px | 통계 숫자 | `55`, `71%`, `15.4` |

`--t-meta`와 `--t-small`은 값이 같지만 역할이 다르다. 보조 줄은 내용(누가, 언제, 어디서)이고
작은 글은 조작·안내(버튼, 캡션, 개수)다. 나중에 한쪽만 바꿔야 할 때를 위해 이름을 구분해 둔다.

**정하는 법** — 새 요소를 만들 때 이 순서로 묻는다.

1. 페이지에 하나뿐인 제목인가 → `--t-page`. 페이지 안의 구역을 여는 제목인가 → `--t-sec`.
2. 반복되는 항목(카드·목록 줄) 안에서 가장 먼저 읽을 줄인가 → `--t-item`, 굵게.
3. 문장으로 읽는 글인가 → `--t-body`. 그 아래 붙는 정보 줄인가 → `--t-meta`.
4. 누르거나 세는 것인가 → `--t-small`. 알약·뱃지·대문자 라벨인가 → `--t-label`.

**같은 역할은 어느 페이지에서나 같은 크기다.** 논문 목록의 제목과 과제 카드의 제목과
구성원 이름은 전부 `--t-item`이다. "이 페이지만 조금 크게"는 없다.

**좁은 화면**은 [extra.css](assets/css/extra.css) 맨 아래 `@media` 블록에서 **변수만** 내린다.
선택자마다 따로 `font-size`를 주지 않는다. 지금은 1100 · 900 · 600px 세 단이다.

**이 표 밖인 것**: 아이콘 글리프(×, ▶, →, Font Awesome). 글자가 아니라 그림이라 px로 둔다.
`em`·`rem`도 쓰지 않는다 — 부모가 바뀌면 크기가 따라 흔들린다.

**확인하는 법** — CSS를 고친 뒤 이 명령이 아이콘 12곳 말고 아무것도 내놓지 않아야 한다.

```bash
grep -n "font-size: *[0-9.]*px" assets/css/*.css
```
