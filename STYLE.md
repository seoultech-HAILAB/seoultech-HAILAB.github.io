...
| 이름 | 값 | 쓰는 곳 |
|---|---|---|
| `--t-hero` | 46px | 첫 화면 제목 하나뿐 |
| `--t-page` | 32px | 페이지 제목 (`Publications`), 글 제목 |
| `--t-sec` | 24px | 구역 제목 (`› Faculty`, `What We Explore`) |
| `--t-item` | 18px | 카드·목록 한 줄의 제목, 논문 제목 — 굵게 |
| `--t-body` | 17px | 본문, 메뉴 |
| `--t-meta` | 15px | 날짜, 저자, 학회명, 역할, 카드 설명 줄 |
| `--t-small` | 15px | 버튼, 캡션, 주석, 개수 (보조와 같은 크기, 역할만 다르다) |
| `--t-label` | 12px | 태그, 뱃지, 대문자 라벨 |
| `--t-num` | 34px | 통계 숫자 (`55`, `71%`) |

좁은 화면에서는 [extra.css](assets/css/extra.css) 맨 아래에서 **변수만** 한 단씩 내린다.
선택자마다 따로 줄이지 않는다. 아이콘 글리프(×, ▶, FA 아이콘)는 글자가 아니라 이 표 밖이다.
